...
impl<'a> Scheduler<'a> {
    ...
    fn init_globals(skel: &mut OpenBpfSkel, opts: &Opts, order: &CpuOrder, debug_level: u8) {
        ...
        if !opts.enable_cpu_bw {
            skel.struct_ops.lavd_ops_mut().cgroup_init = std::ptr::null_mut();
            skel.struct_ops.lavd_ops_mut().cgroup_exit = std::ptr::null_mut();
            skel.struct_ops.lavd_ops_mut().cgroup_move = std::ptr::null_mut();
            skel.struct_ops.lavd_ops_mut().cgroup_set_bandwidth = std::ptr::null_mut();
         } else if !ksym_exists("scx_group_set_bandwidth").unwrap() {
            skel.struct_ops.lavd_ops_mut().cgroup_set_bandwidth = std::ptr::null_mut();
