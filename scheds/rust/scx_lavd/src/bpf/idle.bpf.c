			...
			u64 scope, s64 *sticky_cpdom, bool *is_idle)
{
	...
	bpf_for(i, 0, LAVD_CPDOM_MAX_DIST) {
		...
		bpf_for(j, 0, LAVD_CPDOM_MAX_NR) {
			...
			if (cpu >= 0) {
				/*
				 * Donation redirects a waking task to a
				 * neighbor — the task was never queued in
				 * the stealee domain, so don't decrement
				 * the stealee's budget. Both stealer and
				 * stealee flags stay active for the round.
				 */
